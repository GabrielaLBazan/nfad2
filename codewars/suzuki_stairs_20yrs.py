...
1) Find the sum of all the stairs logged in a year.

2) Find the average amount of stairs climbed in a year. (20_year_estimate = one_year_total * 20)

3) Function will return the 20 year estimate of the stairs based on the average of one year.

4) Each weekday in the stairs array is a nested array.

"""
# confused by the stairs argument, isn't that what I'm looking for?

def stairs_in_20(stairs):

    sun_sum = (sum(sunday))
    mon_sum = (sum(monday))
    tues_sum = (sum(tuesday))
    wed_sum = (sum(wednesday))
    thur_sum = (sum(thursday))
    fri_sum = (sum(friday))
    sat_sum = (sum(saturday))

    one_year_total = sun_sum + mon_sum + tues_sum + wed_sum + thur_sum + fri_sum +sat_sum
...
for test in tests:
    ...
    print "Result: " + str(stairs_in_20(test['input']))

    if stairs_in_20(test['input']) == test['expected']:
